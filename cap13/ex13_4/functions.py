...
def update_drops(ai_settings: Settings, screen: pygame.Surface, drops: Group[Any]):
    """
    ...
    for drop in drops.copy():
        if drop.rect.top >= screen_rect.bottom:
            drops.remove(drop)  # Remove do grupo (destrói o sprite antigo)
            dropped_count += 1
    ...
    if dropped_count > 0:
        ...
        for drop_number in range(number_drops_x):
            create_star(ai_settings, screen, drops, drop_number, row_number=0)
